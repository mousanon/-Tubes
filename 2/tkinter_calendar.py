...
def detailTodo(cb = None):
    ...
    tk.Label(win, text = "Judul:").grid(row = 1, column = 1, sticky = 'N')
...
# s.configure(foreground = "orange")
root.title("Calenderku")

cal = Calendar(root, font = 'Arial 14', selectmode = 'none', locale = 'en_US', cursor = 'hand2', headerbackground = 'white',
            normalbackground = 'white', bordercolor = 'grey', foreground = 'black', weekendbackground = 'red', 
            othermonthbackground = 'light grey', firstweekday = 'sunday'
            )
cal.grid(row = 0, column = 0, sticky = 'E', rowspan = 10)    
cal.bind("<<CalendarSelected>>", ListTodo)
tanggal = str(cal.selection_get())

treev = ttk.Treeview(root)
treev.grid(row = 0, column = 1, sticky = "S", rowspan = 4, columnspan = 2)

scrollBar = tk.Scrollbar(root, orient = 'vertical', command = treev.yview)
scrollBar.grid(row = 0, column = 3, sticky = "ENE", rowspan = 4)
